...
class AdminCmsUsersController extends CBController {
	...
	public function cbInit() {
        ...
        $this->form[] = array("label"=>"Docente","name"=>"docente_id","type"=>"select2","datatable"=>"docente,nombres","datatable_format"=>"nombres,' - ',apellidos,' - ',email","style"=>"hidden");
        ...
        $this->script_js = "(function(){
            $('#form-group-docente_id').hide();

            $('#id_cms_privileges').change(function(e){
                if($(this).val() == 3){
                    $('#form-group-docente_id').show();
                    $('#docente_id').attr('required','required');
                    $('#docente_id').focus();
                }else{
                    $('#form-group-docente_id').hide();
                    $('#docente_id').removeAttr('required');
                }
            });

            $('#docente_id').on('select2:selecting',function(e){
                var docente = e.params.args.data.text;
                 docente = docente.split('-');
                $('#name').val(docente[0]+' '+docente[1]);
                $('#email').val(docente[2]);
            });
